    ...
    fetchWithRetry(apiUrl("/api/blogs"), { cache: "no-store" })
      ...
      .then((data) => {
        if (data.success && Array.isArray(data.data)) {
          // When backend is running, only show dynamic blogs (no static merge)
          setBlogs(data.data);
    ...
    <>
      <div className="max-w-7xl mx-auto px-4 py-10">
        <div className="flex items-center justify-center gap-3 mb-8">
          <h1 className="text-5xl text-center font-bold text-[#FDC017]">Blogs</h1>
          {isRefreshing && (
            <RefreshCw
              size={22}
              className="text-[#FDC017] animate-spin mt-1"
            />
          )}
        </div>
              ...
              return (
                ...
                >
                  <div className="rounded-xl border border-gray-200 bg-white shadow-md hover:shadow-2xl transition-all duration-300 h-full flex flex-col overflow-hidden">
                    ...
                    <h3 className="text-2xl font-bold text-gray-900 mt-2 mb-3 leading-snug group-hover:text-[#FDC017] transition-colors">
                      {post.title}
                    </h3>
