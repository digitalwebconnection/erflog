 ...
 *                   e.g. VITE_API_URL=https://erfolg-backend.onrender.com
 *
 * Usage:
...
export function apiUrl(path: string): string {
  ...
  if (normalizedBase.endsWith('/api') && normalizedPath.startsWith('/api')) {
    normalizedPath = normalizedPath.slice(4); // Remove "/api" prefix from path
  }
...
export function getImageUrl(url: string): string {
  ...
  // If the database stored a backend proxy path like /api/blogs/image/v12345/erfolg/...
