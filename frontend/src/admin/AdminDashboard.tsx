...
import { getCountries, deleteCountry, logout, getBlogs, deleteBlog } from './api';
...
export default function AdminDashboard({ onLogout }: { onLogout: () => void }) {
  ...
  const [section, setSection] = useState<'countries' | 'blogs'>('countries');
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      if (section === 'countries') {
        ...
        setCountries(p => p.filter(c => c._id !== id));
      } else {
        await deleteBlog(id);
  ...
  const handleFormSuccess = () => {
    ...
    showToast(
      section === 'countries'
        ? (view === 'add' ? 'Country created!' : 'Country updated!')
        : (view === 'add' ? 'Blog created!' : 'Blog updated!')
    );
  };
  ...
  if (view === 'add' || view === 'edit') return (
    <div className="h-screen bg-slate-950 flex overflow-hidden">
      <Sidebar
        ...
        onSectionChange={(sec) => { setSection(sec); setView('list'); setEditId(null); }}
        onToggle={() => setSidebarOpen(v => !v)}
        onLogout={() => setLogoutConfirm(true)}
      />
      <div className={`flex-1 flex flex-col transition-all h-screen overflow-hidden ${sidebarOpen ? 'ml-60' : 'ml-16'}`}>
        <TopBar title={section === 'countries' ? (view === 'add' ? 'Add New Country' : 'Edit Country') : (view === 'add' ? 'Add New Blog' : 'Edit Blog')} onToggle={() => setSidebarOpen(v => !v)} />
        <div className="flex-1 p-6 overflow-y-auto" data-lenis-prevent>
          {section === 'countries' ? (
            ...
            <BlogForm
              blog={view === 'edit' ? blogs.find(b => b._id === editId) : null}
              onSuccess={() => handleFormSuccess()}
              onClose={() => { setView('list'); setEditId(null); }}
            />
          )}
        </div>
      </div>
  ...
  return (
    <div className="h-screen bg-slate-950 flex overflow-hidden">
      ...
      />
      <div className={`flex-1 flex flex-col transition-all h-screen overflow-hidden ${sidebarOpen ? 'ml-60' : 'ml-16'}`}>
        <TopBar title={section === 'countries' ? "Country Management" : "Blog Management"} onToggle={() => setSidebarOpen(v => !v)} />
        <div className="flex-1 p-6 overflow-y-auto" data-lenis-prevent>
            ...
            <>
              ...
              <div className="flex items-center justify-between mb-8">
                <div>
                  ...
                  <p className="text-slate-400 text-sm mt-1">{blogs.length} blogs in database</p>
                </div>
                <button onClick={() => setView('add')}
                  className="flex items-center gap-2 bg-gradient-to-r from-amber-500 to-blue-600 text-white font-semibold px-5 py-2.5 rounded-xl hover:scale-105 transition shadow-lg text-sm">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" width="16" height="16"><line x1="12" y1="5" x2="12" y2="19" /><line x1="5" y1="12" x2="19" y2="12" /></svg>
              ...
              {loading ? (
                ...
                <div className="flex flex-col items-center justify-center py-32 text-slate-400">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" width="56" height="56" className="mb-4 opacity-40">
                    <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z" /><path d="M16 8h2M16 12h2M16 16h2M6 8h6v8H6z" />
                  </svg>
                  <p className="mb-4">No blogs yet.</p>
                  <button onClick={() => setView('add')} className="bg-gradient-to-r from-amber-500 to-blue-600 text-white px-6 py-2 rounded-xl font-semibold text-sm">Add First Blog</button>
                </div>
              ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
                  {blogs.map(b => (
                    <div key={b._id} className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden hover:border-slate-600 hover:-translate-y-1 transition-all group flex flex-col justify-between">
                      <div>
                        <div className="relative h-44 bg-slate-800">
                          {b.image
                            ? <img src={getImageUrl(b.image)} alt={b.title} className="w-full h-full object-cover group-hover:scale-105 transition duration-500" />
                            : <div className="w-full h-full flex items-center justify-center text-slate-600">
                              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" width="48" height="48"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z" /><path d="M16 8h2M16 12h2M16 16h2M6 8h6v8H6z" /></svg>
                            </div>
                          }
                          <span className="absolute top-3 right-3 bg-black/60 backdrop-blur text-white text-xs font-bold px-2 py-1 rounded-lg">{b.categories || 'General'}</span>
                        </div>
                        <div className="p-4">
                          <h3 className="text-white font-semibold text-base line-clamp-2">{b.title}</h3>
                          <p className="text-slate-400 text-xs mt-1">📅 {b.date || 'No Date'}</p>
                          <p className="text-slate-500 text-xs mt-2 line-clamp-3">{b.excerpt || 'No description.'}</p>
                        </div>
                      </div>
                      <div className="flex border-t border-slate-800">
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center" onClick={() => setDeleteConfirm(null)}>
          <div className="bg-slate-900 border border-slate-700 rounded-2xl p-8 w-full max-w-sm mx-4 shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <h3 className="text-white font-bold text-xl text-center mb-2">Delete {section === 'countries' ? 'Country' : 'Blog'}?</h3>
      ...
      {logoutConfirm && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center" onClick={() => setLogoutConfirm(false)}>
          <div className="bg-slate-900 border border-slate-700 rounded-2xl p-8 w-full max-w-sm mx-4 shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-center w-14 h-14 bg-amber-500/10 rounded-full mx-auto mb-4">
            ...
            <h3 className="text-white font-bold text-xl text-center mb-2">Confirm Logout?</h3>
            <p className="text-slate-400 text-sm text-center mb-6">Are you sure you want to log out of the admin panel?</p>
            <div className="flex gap-3">
              <button onClick={() => setLogoutConfirm(false)} className="flex-1 py-2.5 rounded-xl border border-slate-700 text-slate-300 hover:bg-slate-800 transition text-sm">Cancel</button>
              <button onClick={() => { logout(); onLogout(); }}
                className="flex-1 py-2.5 rounded-xl bg-red-600 hover:bg-red-500 text-white font-semibold transition text-sm">
                Log Out
              </button>
            </div>
          </div>
...
function Sidebar({
  ...
  open: boolean;
  currentSection: 'countries' | 'blogs';
  onSectionChange: (sec: 'countries' | 'blogs') => void;
  onToggle: () => void;
  onLogout: () => void;
}) {
  return (
    <aside className={`fixed top-0 left-0 h-full bg-slate-900 border-r border-slate-800 flex flex-col z-40 transition-all ${open ? 'w-60' : 'w-16'}`}>
      <div className={`flex items-center gap-3 px-4 py-5 border-b border-slate-800 ${open ? '' : 'justify-center'}`}>
        ...
        {open && <span className="text-white font-bold text-sm">Erfolg</span>}
      </div>
      <nav className="flex-1 p-3 space-y-2">
        <button
          onClick={() => onSectionChange('countries')}
          className={`flex items-center gap-3 w-full px-3 py-2.5 rounded-xl border text-left cursor-pointer transition ${
            currentSection === 'countries'
              ? 'bg-gradient-to-r from-amber-500/10 to-blue-600/10 border-amber-500/20 text-amber-400'
        ...
        >
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="18" height="18"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z" /><path d="M16 8h2M16 12h2M16 16h2M6 8h6v8H6z" /></svg>
