...
const S: Record<string, React.CSSProperties> = {
  ...
  body: { padding: '1.5rem 1.75rem', display: 'flex', flexDirection: 'column', gap: '1.25rem', background: '#0f172a' },
  section: {
  ...
  label: {
    ...
    color: '#cbd5e1', marginBottom: '0.4rem',
  },
  input: {
    ...
    background: '#1e293b', transition: 'border-color 0.18s',
  },
  textarea: {
    width: '100%', padding: '0.6rem 0.75rem',
    border: '1px solid #334155', borderRadius: '12px',
    fontSize: '0.9rem', color: '#ffffff',
    outline: 'none', boxSizing: 'border-box',
    background: '#1e293b', resize: 'vertical', minHeight: '80px',
    fontFamily: 'inherit',
  },
  imgPreview: {
    marginTop: '0.6rem', width: '100%', maxHeight: '150px',
    objectFit: 'cover', borderRadius: '6px', border: '1px solid #1e293b',
  },
  footer: {
  ...
  cancelBtn: {
    padding: '0.55rem 1.25rem', border: '1px solid #334155',
    borderRadius: '12px', background: 'transparent', cursor: 'pointer',
    fontSize: '0.85rem', color: '#cbd5e1', fontWeight: 600,
  },
  saveBtn: {
...
export default function BlogForm({ blog, onSuccess, onClose }: Props) {
  ...
  const [imgPreview, setImgPreview] = useState(blog?.image ? getImageUrl(blog.image) : '');
