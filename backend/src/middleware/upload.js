 ...
 * @param {string} folder   - Cloudinary folder (e.g. 'erfolg/images')
 * @returns {Promise<object>} Cloudinary upload result (contains .secure_url, .public_id etc.)
 */
const uploadToCloudinary = (buffer, folder = 'erfolg/images') => {
 ...
 */
const handleImageUpload = (folder = 'erfolg/images') => [
  upload.single('image'),
  async (req, res, next) => {
